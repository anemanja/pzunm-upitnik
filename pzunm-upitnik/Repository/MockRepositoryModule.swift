...
public final class QuestionsRepository: NMQuestionsRepository {
    public func getQuestions(for language: NMLanguage) async -> Result<NMQuestionnaire, GenericError> {
        .success(NMQuestionnaire(
            ...
            questions: [
                NMQuestion(formulation: "Da li ste imali ili imate neku prirodnu ili stečenu bolest organa vida?", answer: false),
                NMQuestion(formulation: "Da li ste imali ili imate urođene ili stečene bolesti organa sluha ili ravnoteže?", answer: false),
                NMQuestion(formulation: "Da li bolujete od neke duševne bolesti?", answer: false),
                NMQuestion(formulation: "Da li ste bolovali ili bolujete od neke bolesti zavisnosti (alkoholizam, toksikomanija, zloupotreba psihotropnih supstanci)?", answer: false)
            ]))
    }
}
